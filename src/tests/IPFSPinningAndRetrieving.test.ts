import {
  uploadToIPFS,
  retrieveFromIPFS,
} from "../utils/IPFSPinningAndRetrieving.ts";
import { encryptData, decryptData } from "../utils/encryptAndDecrypt.ts";
...
const data = `{
	...
	"ingredient" : [{ // Composition information about the substance
	  ...
	  "substanceCodeaâbleaaCaaaoaaaaââancepaaat" : { CodeableConcept },
...
async function test() {
  try {
    ...
    if (typeof url !== "string" || !url) {
      throw new Error("Failed to upload to IPFS: URL is undefined or empty");
    }
